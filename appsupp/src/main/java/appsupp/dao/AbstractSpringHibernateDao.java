...
public abstract class AbstractSpringHibernateDao extends HibernateDaoSupport {
	...
	public DomainCode getDomainCode(final int dcId) {
		Criteria criteria = getHibernateTemplate().getSessionFactory()
				.getCurrentSession().createCriteria(DomainCode.class);
	...
	public Integer getDomainCodeId(String domainName, String domainCode) {
		DomainCode domainCodeObject = this.getDomainCode(domainName,
				domainCode);
	...
	public DomainCode getDomainCodeByDesciption(String domainName,
	...
	private DomainCode getDomainCode(String domainName, String domainCode,
			boolean searchByDomainCode) {
		List<DomainCode> rows = getDomainCodes(domainName, domainCode,
				searchByDomainCode);
		DomainCode domainCodeObject = null;
		if (rows.size() > 0)
			domainCodeObject = (DomainCode) rows.get(0);
	...
	@SuppressWarnings("unchecked")
	private List<DomainCode> getDomainCodes(String domainName,
			String domainCode, boolean searchByDomainCode) {
		// get the id for the given domain name + code (string)
		Criteria criteria = getHibernateTemplate().getSessionFactory()
				.getCurrentSession().createCriteria(DomainCode.class, "dc");
